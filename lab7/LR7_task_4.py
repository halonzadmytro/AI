...
class CityMap:
    def __init__(self, distances_matrix, cities_count):
        ...
        self.pheromones = [[np.random.rand() for j in range(cities_count)]
...
class Colony:
    ...
    def find_route(self, city_map, city_num):
        ...
        for cycle in range(self.maxColonyCycles):
            pheromones_delta = [[0.0 for i in range(
                city_map.numberOfCities)] for j in range(city_map.numberOfCities)]
            for antNumber in range(self.numberOfAnts):
                ...
                while len(ant.visitedCities) < city_map.numberOfCities:
                    ...
                    ant.move(
                        next_city, city_map.distances[ant.currentCity][next_city])
            ...
            for city in range(len(ant.visitedCities) - 1):
                ...
                city_map.upd_pheromones(
                    self.pheromoneEvaporationRate, pheromones_delta)
    ...
    def get_probabilities(self, ant, city_map):
        ...
        for newCity in range(city_map.numberOfCities):
            if (newCity != ant.currentCity) and (self.antCanVisitPreviousCities or newCity not in ant.visitedCities):
                probability = pow(city_map.pheromones[ant.currentCity][newCity], self.pheromoneImportance) * pow(1 / city_map.distances[ant.currentCity][newCity],
                                                                                                                 self.distanceImportance)
                result[newCity] = probability
                total_probability += probability
        result = [result[i] /
                  total_probability for i in range(city_map.numberOfCities)]
        return result
...
cities = [
    ...
    'Івано-Франківськ', 'Київ', 'Кропивницький', 'Луганськ', 'Луцьк', 'Львів', 'Миколаїв', 'Одеса',
...
if __name__ == '__main__':
    ...
    plt.plot([i + 1 for i in range(25)], result[1], ms=12, marker='*',
             mfc='r', mec='black', mew=2, color='black', ls="--")
    plt.grid()
    plt.show()
